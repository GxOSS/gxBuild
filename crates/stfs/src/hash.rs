...
impl HashTableMeta {
    ...
    pub fn read_top_table<R: ReadAt>(&mut self, source: &R) -> Result<(), StfsError> {
        ...
        for _ in 0..self.top_table.entry_count {
            ...
            self.top_table.entries.push(HashEntry {
                block_hash: Sha1Digest(hash_bytes),
                status,
                next_block,
            });
        }
    ...
    pub(crate) fn compute_first_level_backing_hash_block_number(
        &self,
        block: usize,
        sex: StfsPackageSex,
    ) -> usize {
    ...
    pub(crate) fn compute_second_level_backing_hash_block_number(
        &self,
        block: usize,
        sex: StfsPackageSex,
    ) -> usize {
    ...
    fn compute_data_block_num(&self, block: usize, sex: StfsPackageSex) -> usize {
        ...
        if block < HASHES_PER_HASH_TABLE {
            ...
            (addr
                + ((addr + DATA_BLOCKS_PER_HASH_TREE_LEVEL[2])
    ...
    pub(crate) fn hash_table_skip_for_address(
        &self,
        table_address: usize,
        sex: StfsPackageSex,
    ) -> usize {
    ...
    pub(crate) fn block_hash_address<R: ReadAt>(
        ...
        let mut hash_addr = (self.compute_first_level_backing_hash_block_number(block, sex)
            * BLOCK_SIZE)
        ...
        match self.top_table.level {
            ...
            HashTableLevel::Third => {
                let first_level_offset = ((self.top_table.entries
                    [block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]]
                    .status as usize)
                    & 0x40)
                    << 6;

                let position = (self.compute_second_level_backing_hash_block_number(block, sex)
                    * BLOCK_SIZE)
                    + self.first_table_address
                    + first_level_offset
