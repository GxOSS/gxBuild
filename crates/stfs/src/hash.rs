...
impl Default for HashTable {
    fn default() -> Self {
        Self { level: HashTableLevel::First, true_block_number: 0, entry_count: 0, address_in_file: 0, entries: Vec::new() }
    }
}
...
impl HashTableMeta {
    pub fn from_header(header: &XContentHeader, sex: StfsPackageSex) -> Result<Self, StfsError> {
        ...
        let tables_per_level = [
            (allocated_block_count / HASHES_PER_HASH_TABLE) + if !allocated_block_count.is_multiple_of(HASHES_PER_HASH_TABLE) { 1 } else { 0 },
            0,
            0,
        ];
        ...
        let mut meta = HashTableMeta { block_step, tables_per_level, first_table_address, top_table: HashTable { level: top_level, ..Default::default() } };
        ...
        if (allocated_block_count > DATA_BLOCKS_PER_HASH_TREE_LEVEL[2] && !allocated_block_count.is_multiple_of(DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]))
            || (allocated_block_count > HASHES_PER_HASH_TABLE && !allocated_block_count.is_multiple_of(HASHES_PER_HASH_TABLE))
        {
    ...
    pub(crate) fn compute_backing_hash_block_number_for_level(&self, block: usize, level: HashTableLevel, sex: StfsPackageSex) -> usize {
        match level {
    ...
    fn compute_data_block_num(&self, block: usize, sex: StfsPackageSex) -> usize {
        ...
        if block < HASHES_PER_HASH_TABLE {
            ...
            (1 << sex as usize) + ((addr + ((block + DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]) / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2])) << sex as usize)
        }
    }
    ...
    pub fn read_block_hash_entry<R: ReadAt>(&self, source: &R, block: BlockNumber, sex: StfsPackageSex, stfs_vol: &StfsVolumeDescriptor) -> Result<HashEntry, StfsError> {
    ...
    pub(crate) fn block_hash_address<R: ReadAt>(&self, block: usize, sex: StfsPackageSex, stfs_vol: &StfsVolumeDescriptor, source: &R) -> Result<usize, StfsError> {
        ...
        let mut hash_addr = (self.compute_first_level_backing_hash_block_number(block, sex) * BLOCK_SIZE) + self.first_table_address;
        ...
        match self.top_table.level {
            ...
            HashTableLevel::Third => {
                let first_level_offset = ((self.top_table.entries[block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]].status as usize) & 0x40) << 6;
