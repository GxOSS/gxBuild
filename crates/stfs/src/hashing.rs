...
impl StfsHasher {
    ...
    pub fn verify_data_block<R: ReadAt>(&mut self, source: &R, data_block: BlockNumber, ctx: &HashVerifyContext) -> Result<Vec<BlockHashReport>, StfsError> {
        ...
        match top_level {
            HashTableLevel::Third => {
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::Third, &ctx.stfs_vol.top_hash_table_hash, ctx)?);

                let second_expected = &ctx.hash_meta.top_table.entries[data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]].block_hash;
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::Second, second_expected, ctx)?);

                let first_expected = self.read_hash_entry_at_level(source, data_block, HashTableLevel::Second, ctx)?;
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::First, &first_expected, ctx)?);
            }
            HashTableLevel::Second => {
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::Second, &ctx.stfs_vol.top_hash_table_hash, ctx)?);

                let first_expected = &ctx.hash_meta.top_table.entries[data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]].block_hash;
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::First, first_expected, ctx)?);
            }
            HashTableLevel::First => {
                reports.push(self.verify_hash_table_block(source, data_block, HashTableLevel::First, &ctx.stfs_vol.top_hash_table_hash, ctx)?);
            }
        }
    ...
    pub fn verify_data_block_content<R: ReadAt>(&self, source: &R, data_block: BlockNumber, ctx: &HashVerifyContext) -> Result<BlockHashReport, StfsError> {
    ...
    fn verify_hash_table_block<R: ReadAt>(
        ...
        let report = BlockHashReport { block: hash_block_num, level, calculated_hash: calculated, expected_hash: *expected_hash, is_valid: calculated == *expected_hash };
    ...
    fn read_hash_entry_at_level<R: ReadAt>(&self, source: &R, data_block: usize, level: HashTableLevel, ctx: &HashVerifyContext) -> Result<Sha1Digest, StfsError> {
        let hash_block_num = ctx.hash_meta.compute_backing_hash_block_number_for_level(data_block, level, ctx.sex);
