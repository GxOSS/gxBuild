...
impl StfsHasher {
    ...
    pub fn verify_data_block<R: ReadAt>(
        ...
        match top_level {
            HashTableLevel::Third => {
                ...
                let second_expected = &ctx.hash_meta.top_table.entries
                    [data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]]
                    .block_hash;
                reports.push(self.verify_hash_table_block(
                    source,
                    data_block,
                ...
                let first_expected =
                    self.read_hash_entry_at_level(source, data_block, HashTableLevel::Second, ctx)?;
                reports.push(self.verify_hash_table_block(
                    source,
                    data_block,
            ...
            HashTableLevel::Second => {
                ...
                let first_expected = &ctx.hash_meta.top_table.entries
                    [data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]]
                    .block_hash;
                reports.push(self.verify_hash_table_block(
                    source,
                    data_block,
    ...
    pub fn verify_data_block_content<R: ReadAt>(
        ...
        let entry =
            ctx.hash_meta
                .read_block_hash_entry(source, data_block, ctx.sex, ctx.stfs_vol)?;
    ...
    fn verify_hash_table_block<R: ReadAt>(
        ...
        let hash_block_num = ctx
            .hash_meta
            .compute_backing_hash_block_number_for_level(data_block, level, ctx.sex);
        ...
        let base_addr = (hash_block_num * BLOCK_SIZE)
            + ctx.hash_meta.first_table_address
            + (active_index << 12);
    ...
    fn active_index_for_level(
        ...
        match (ctx.hash_meta.top_table.level, level) {
            (l, target) if l as u8 == target as u8 => {
                ((ctx.stfs_vol.block_separation as usize) & 2) >> 1
            }
            (HashTableLevel::Second, HashTableLevel::First) => {
                let entry = &ctx.hash_meta.top_table.entries
                    [data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]];
    ...
    fn read_hash_entry_at_level<R: ReadAt>(
        ...
        let entry_index = match level {
            ...
            HashTableLevel::Second => {
                (data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[1]) % HASHES_PER_HASH_TABLE
            }
            HashTableLevel::Third => {
                (data_block / DATA_BLOCKS_PER_HASH_TREE_LEVEL[2]) % HASHES_PER_HASH_TABLE
            }
        };
